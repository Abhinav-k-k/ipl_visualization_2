...
fetchAndVisualizeData();
  ...
  function clicked(event){
    ...
    fetch(`/economy?year=${year}`)
      .then(re => re.json())
      .then(data => {
        visualize_economicalBowlersCopy(data,year);
      });
  };
...
function visualize_economicalBowlers(economicalBowlers) {
  ...
  for (let year in economicalBowlers) {
    if(year==2015){
      for (let name in economicalBowlers[year]){
        seriesData.push([name, economicalBowlers[year][name]]);
      }
    }
  }
...
// -------------------------------------------------------------------------------------------------------------------------
function visualize_economicalBowlersCopy(data,year) {
  const seriesData = [];
  for (let name in data) {
    seriesData.push([name, data[name]]);
  }

  Highcharts.chart("economical_bowlers1", {
    chart: {
      type: "column"
    },
    title: {
      text: `Economical bowlers in ${year}`
    },
    subtitle: {
      text:
        'Source: <a href="https://www.kaggle.com/nowke9/ipldata/data">IPL Dataset</a>'
